import { formatFileNameForBucket } from "./utils/formatFileNameForBucket"
import supabaseClient from "@/libs/supabaseClient"
import { TDBBuckets } from "@/ts/types/TDBBuckets"
...
/**
 *
 * @param imageFile - File type (make sure it exist and not undefined - could be null to make code conciser)
 * @param folder - folder (category) e.g Main dishes
 * @param suffix - something after file name e.g fileName_price_id_d4rg3f2d
 * @returns - string if error - null if no image - publicUrl for uploaded image
...
export async function uploadImageFn({
  ...
  folder,
  suffix,
}: UploadImageParams): Promise<string | null | { publicUrl: string }> {
  ...
  // 2. Fetch existing files in the folder
  const { data: existingFiles, error: listError } = await supabaseClient.storage.from(bucket).list(folderPath)
  if (listError) return `Can't list files in ${folderPath || `Supabase bucket "tickets-images"`} – ${listError.message}`
